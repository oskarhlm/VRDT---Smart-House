...
# response = requests.get('https://www.hvakosterstrommen.no/api/v1/prices/2023/04-19_NO3.json')
response = requests.get('https://www.hvakosterstrommen.no/api/v1/prices/' + date_to_string + '_NO3.json')
...
for element in json_response:
     nok_per_kWh.append(element['NOK_per_kWh'])
...
def calculate_irr(panel_tilt=25, panel_azimuth=180, time=now):
    ...
    current_irradiance = get_irradiance(site, str(time.date()), panel_tilt, panel_azimuth)
...
def approximated_outp(model=SPR_Max3_400, temp=20, num_panels=1, panel_tilt=25, panel_azimuth=18, time=now, rounded=rounded, time_rounded=time_rounded):
    #calculate_irr takes inn solar panel tilt and azimuth degree as arguments
    summer_irradiance, current_irradiance = calculate_irr(panel_tilt, panel_azimuth, time)

    temp_eff = panel_temp_efficiency(model, temp)
#     print('Approx efficiency at {t} \N{DEGREE SIGN}C: {e:2f} %'.format(t=temp, e=temp_eff*100))

    current_power_outp = current_irradiance.at[time_rounded, 'POA'] * temp_eff * (model['panel_size']*num_panels)
    
    daily_power_outp = current_irradiance['POA'] * temp_eff * (model['panel_size']*num_panels)
...
def plot_irradiance(comparison=False, model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    time_rounded = rounded.strftime('%H:%M')
    _, _, summer_irradiance, current_irradiance, _, _, _, _, _= setup(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
    if comparison:
...
def plot_irr_vs_pow(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    _, daily_approx_power, _, current_irradiance, _, _, _, _, _= setup(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
    ...
    axs[1].plot(x_seq, daily_approx_power, label='Power output {txt} panels'.format(txt=num_panels), color='red')
...
def plot_current_pow(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    current_approx_power, _, _, _, temp_eff, _, _, _, _ = setup(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
...
def display_statistics(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    current_approx_power, _, _, current_irradiance, _, _, total_power_gen, _, total_money_gen = setup(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
    POA_irradiance = current_irradiance.at[time_rounded, 'POA']
    print('#---------#')
    print(' --STATS-- ')
    print('#---------#')
    print('Solar panel model                                           {txt}'.format(txt=model['name']))
    print('Number of panels                                            {txt}'.format(txt=num_panels))
    print(' ')
    print('Current POA irradiance ({t}):               {txt:.2f} W/m^2'.format(t=(rounded), txt=POA_irradiance))
    print('Irradiance offset from optimal solarpanel tilt (GHI - POA): {txt:.2f} W/m^2'.format(txt=np.abs(current_irradiance.at[time_rounded, 'GHI'] - POA_irradiance)))
    print(' ')
    print('Current power output   ({t}):               {txt:2f} W'.format(t=(rounded),txt=current_approx_power))   
    print('Total power generated          {date}:               {power:2f} kWh'.format(date=(time.year, time.month, time.day), power=total_power_gen/1000))
    print('Total money generated          {date}:               {money:2f} Kr'.format(date=(time.year, time.month, time.day), money=total_money_gen))
    print('#---------#')
...
def display_current_power(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    current_approx_power, _, _, _, _, _, _, _, _ = setup(model, temp, num_panels, panel_tilt, panel_azimuth, time)
...
def display_power_total(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    _, _, _, _, _, _, total_power_gen, _, _ = setup(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
    print('Total power generated {date}: {power:2f} kWh'.format(date=(time.year, time.month, time.day), power=total_power_gen/1000))
...
def display_money_total(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now):
    ...
    _, _, _, _, _, _, _, _, total_money_gen = setup(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
    print('Total money generated {date}: {money:2f} Kr'.format(date=(time.year, time.month, time.day), money=total_money_gen))
...
def setup(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now, rounded=rounded, time_rounded=time_rounded):
    ...
    current_approx_power, daily_approx_power, summer_irradiance, current_irradiance, temp_eff = approximated_outp(model, temp, num_panels, panel_tilt, panel_azimuth, time, rounded, time_rounded)
...
plot_current_pow(model=SPR_Max3_400, temp=20, num_panels=4, panel_tilt=25, panel_azimuth=180, time=now)
